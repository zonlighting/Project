...
public class UserDao {
  ...
  public void createUser(User user) {
    String sql =
        "INSERT INTO usertbl (username, password,name,age,sex,address,avatar,email) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    jdbcTemplate.update(sql, user.getUserName(), user.getPassword(), user.getName(), user.getAge(),
        user.getSex(), user.getAddress(), user.getAvatar(), user.getEmail());
  ...
  public boolean checUser(String userName) {
    ...
    List<User> users =
        jdbcTemplate.query(sql, new BeanPropertyRowMapper<User>(User.class), userName);
    if (users.size() > 0) {
      return true;
    } else {
      return false;
  ...
  public void changeInfo(User user) {
    String sql =
        "UPDATE usertbl SET name = ?, age = ?, address = ?, sex = ?, avatar = ? WHERE username = ? ";
    jdbcTemplate.update(sql, user.getName(), user.getAge(), user.getAddress(), user.getSex(),
